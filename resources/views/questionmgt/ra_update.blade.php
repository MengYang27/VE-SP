...
$infoFromDB = SelectSigleRow('rainfo','raid', $_id);
// var_dump($infoFromDB);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $audioPaths_array = string_to_array(",", $infoFromDB[0][17]); // From Database
      ...
      <ul id="sortable2" class="connectedSortable">
        <?php
            ...
            foreach ($currentQuestion->categories_array as $component) {
                $splited_array = string_to_array('-', $component);
                // var_dump($splited_array);
                echo '<li class="ui-state-default"'.'value="' . $splited_array[0] . '-' . $splited_array[1] . '"'.'>'.$categories_secondary[$splited_array[0]][$splited_array[1]].'</li>';
            }
        }
        ?>
      </ul>
